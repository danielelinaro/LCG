\documentclass[10pt,a4paper,oneside]{book}

\usepackage[utf8]{inputenc}
\usepackage[english]{babel}
\usepackage[english]{isodate}
\usepackage[parfill]{parskip}
\usepackage{amsthm}
\usepackage{graphicx} 
\usepackage{tikz}
\usetikzlibrary{patterns}
%\graphicspath {{figures/}}
\usepackage{hyperref}
\usepackage{natbib}
\usepackage{float}
\usepackage{caption}
% Use listings instead of verbatim (allows word wrapping)
\usepackage{xcolor}
\usepackage{xspace}
\usepackage{listings}
\usepackage{textcomp} % for the single quotes on the listings
\usepackage[squaren]{SIunits}
% tables
\usepackage{tabularx}
\usepackage{booktabs}
\newcommand{\head}[1]{\textnormal{\textbf{#1}}}
\usepackage{array}
\usepackage{float}
% chaptersection
\usepackage{minitoc}
% margin notes using the geometry package
\usepackage{marginnote}
\usepackage[top=4cm, bottom=3cm, outer=3cm, inner=3cm, heightrounded,
marginparwidth=2.75cm, marginparsep=0.5cm]{geometry}

\definecolor{light_gray}{RGB}{230,230,230}
\definecolor{dark_green}{RGB}{50,150,50}
\lstset{ 
  backgroundcolor=\color{light_gray},
  % the size of the fonts that are used for the code
  basicstyle = \footnotesize\ttfamily,       
% sets if automatic breaks should only happen at whitespace
  breakatwhitespace = false,        
  breaklines=true,
% sets the caption-position to bottom
  captionpos=b, 
  commentstyle=\color{dark_green},
  frame=none,      
  keywordstyle=\color{blue},
  language=bash,               
  numbers=left,                 
% how far the line-numbers are from the code 
  numbersep=3pt,              
% the style that is used for the line-numbers
  numberstyle=\tiny\color{orange},
% if not set, the frame-color may be changed on line-breaks within not-black text (e.g. comments (green here))  
  rulecolor= \color{white},       
% the step between two line-numbers. If it's 1, each line will be numbered 
  stepnumber=1,                   
% string literal style
  stringstyle=\color{orange},      
  tabsize=4,
  deletekeywords={local,env},
  literate={\$}{{{\$}}}1
}

\newcommand{\inlineCode}{\lstinline[basicstyle=\normalsize\ttfamily]}
\newcommand{\progname}{\textsc{lcg}\xspace}
\newcommand{\matlab}{Matlab\textsuperscript{\textregistered}\xspace}
\newcommand{\unix}{UNIX\xspace}
\newcommand{\comedi}{COMEDI\xspace}
\newcommand{\hdf}{HDF5\xspace}
\newcommand{\sidenote}[1]{\marginnote{\textit{#1}}}

\title{\progname \\ command line electrophysiology \\ A user manual}
\author{Daniele Linaro \\ Jo\~ao Couto \\ Michele Giugliano}
\date{\today}
\hypersetup{
 pdftitle = {\progname, command-line electrophysiology},
 pdfkeywords = {electrophysiology,dynamic clamp,real-time},
 pdfauthor = {(Daniele Linaro, Joao Couto and Michele Giugliano)}
}

\begin{document}

\maketitle
\thispagestyle{empty}

\pagenumbering{roman}
\dominitoc
\tableofcontents
\newpage
\pagenumbering{arabic}

\chapter{Introduction}
\label{chap:intro}
\input{chapters/introduction}

\chapter{Installation}
\label{chap:installation}
\input{chapters/installation}

\chapter{Getting started}
\label{chap:start}
\input{chapters/getting_started}

\chapter{Electrophysiology protocols}
\label{chap:protocols}
\input{chapters/protocols}

\chapter{Stimulus generator}
\label{chap:stimgen}
\input{chapters/stimgen}

\chapter{Data files}
\label{chap:datafiles}
\input{chapters/data_files}

\chapter{Configuration files}
\label{chap:configuration}
\input{chapters/configuration_files}

%\chapter{Streams}
%\label{chap:streams}
%\input{chapters/streams}

\chapter{Entities}
\label{chap:entities}
\input{chapters/entities}

\chapter{Additional features}
\label{chap:features}
\input{chapters/features}

\chapter*{References}
\bibliography{manual}
\bibliographystyle{apsrev}

\appendix
%\chapter{Stimulus Generator Library}
%\input{stimgen/documentation}

\chapter{MATLAB reference}
\label{appendix:matlabReference}
\input{chapters/matlab}

\chapter{UNIX reference}
\label{appendix:unixReference}
\input{chapters/unix}

\end{document}
