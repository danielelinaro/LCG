...
The full list of available protocols (called \emph{commands}, in \progname terminology) is described in
...
\inlineCode{$HOME/local/lib/python2.7/site-packages}, which needs to
be permanently added to your \inlineCode{PYTHONPATH} environment variable, by
adding the previous \verb+export+ line to your \inlineCode{.bashrc} file.
...
\section{LCG environment variables}
\label{sec:envvar}
...
\inlineCode{lcg-env.sh}, provide necessary defaults
...
\progname provides a script that helps the
...
\end{lstlisting}
where the \inlineCode{--CC-channels} and \inlineCode{--VC-channels}
