...
config_file = 'triggered_stimulus.xml'

def usage():
    print('This program waits for a trigger and applies the stimulation described by one or more stimulus files')
    print('to the specified output channel(s) (optional), while at the same time recording from the')
    ...
    for o,a in opts:
        ...
        ############ DIGITAL/TRIGGER ############
            ...
            if a.lower() == 'none':
                ...
                digitalChannels = range(int(a.split('-')[0]),int(a.split('-')[1])+1)
            elif ',' in a:
                # a list of input channels
                for chan in a.split(','):
                    digitalChannels.append(int(chan))
            else:
                # just one input channel
                digitalChannels.append(int(a))
        #########################################
