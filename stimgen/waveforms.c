...
double how_long_lasts_trial(double **vector, INT nlines) {
/*
  double DT, T = 0.;
  while (nlines > 0) {   
...
int simple_waveform(double *vector, double *output, INT *index, INT Ni, double srate, double dt, int verbose) {
long oldseed;
...
if (vector[FIXSEED]) oldseed = mysrand49((long) vector[MYSEED]);
 ...
 switch ((INT) vector[CODE])   // Main decision stage to rule out the requested subwvform type (gauss, DC, etc..).
    {
...
int composite_waveform(double **parsed_data, INT current_line, double *output, INT *index, INT N, double srate, double dt, int verbose) {
INT i, Ni, line = 0, index_old, myindex, howmany;
double *out_temp, *vector;
int return_code;
long oldseed;

Ni       = (INT) (parsed_data[current_line][DURATION] * srate);
out_temp = (double *) calloc(Ni, sizeof(double));
if (out_temp == NULL) { error("Unable to allocate memory for <out_temp> !", verbose);  return -1; }

index_old= (*index);

howmany = (INT) (-parsed_data[current_line][CODE]);
...
//
void DC(double amplitude, double *output, INT *index, INT Ni, double expon, double srate, double dt)
{
  INT i;
...
//
void GAUSS(double mean, double stdv, double *output, INT *index, INT Ni, double expon, double srate, double dt)
...
//
void ORUHL(double mean, double stdv, double tau, double *output, INT *index, INT Ni, double expon, double srate, double dt)
{
...
//
void SINE(double amplitude, double frequency, double phase, double *output, INT *index, INT Ni, double expon, double srate, double dt)
{
 INT i;
 double tmp;
...
//
void POISSON_SHOT1(double amplitude, double frequency, double width, double *output, INT *index, INT Ni, double expon, double srate, double dt)
{
 double tmp;
 INT i,o,m,j,k;
 ...
 if ((expon == -1) || (expon == 0))
  ...
  m   = (INT) (width * srate / 1000.); // How many points constitute the pulse width ? ([width] = ms)
...
//
void BIPOLAR_SHOT(double amplitude, double frequency, double width, double *output, INT *index, INT Ni, double expon, double srate, double dt)
{
 double tmp;
 INT i,o,m,j,k;
...
//
void POISSON_SHOT2(double amplitude, double frequency, double tau, double *output, INT *index, INT Ni, double expon, double srate, double dt)
{
 ...
 // printf("POISSON SHOT 2: an invalid tau has been introduced (tau < dt). Now, tau := 10 dt.");
  ...
  SHOT = (double *) malloc(5*(INT)(fabs(frequency)*Ni*dt) * sizeof(double));
...
//
void RAMP(double y_final, double *output, INT *index, INT Ni, double expon, double srate, double dt)
{
 INT i;
 double tmp1, tmp2;
...
// OK TESTED
void SQUARE(double amplitude, double frequency, double dutycycle, double *output, INT *index, INT Ni, double expon, double srate, double dt)
{
...
// OK TESTED 
void SAW(double amplitude, double frequency, double dutycycle, double *output, INT *index, INT Ni, double expon, double srate, double dt)
{ 
 INT i;
...
// OK TESTED
void SWEEP(double amplitude, double frequency_start, double frequency_stop, double *output, INT *index, INT Ni, double expon, double srate, double dt)
{
 INT i;
 double tmp1, tmp2, tmp3;
...
//
void UNIFNOISE(double mean, double stdv, double *output, INT *index, INT Ni, double expon, double srate, double dt)
{                                          // This is supposed to be a 'white' gaussian noise!
	INT i;
	if (stdv == 0.) {                        // When the specified stddev is 0., the user meant a DC signal.
		DC(mean, output, index, Ni, expon, srate, dt);    // so I think this is going to be quickly generated by DC().
