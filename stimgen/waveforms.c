...
//
void SINE(double amplitude, double frequency, double phase, double offset, double *output, uint *index, uint Ni, double expon, double srate, double dt)
{
 uint i;
 double tmp;

 if (frequency == 0.) {                   // When the specified frequency is 0., the user meant a DC signal.
  DC(amplitude+offset, output, index, Ni, expon, srate, dt);// so it is going to be quickly generated by DC().
  return;
 }
 ...
 if (expon == -1) 
  for (i=0; i<Ni; i++)              // I go through the entire data vector and I initialize it appropriately.
    output[(*index)++] = fabs(offset + amplitude * sin(  tmp * i * dt + phase ));
 else if (expon == 0)
  for (i=0; i<Ni; i++)              // I go through the entire data vector and I initialize it appropriately.
    output[(*index)++] = POSPART(offset + amplitude * sin(  tmp * i * dt + phase ));
 else if (expon != 1)
  for (i=0; i<Ni; i++)              // I go through the entire data vector and I initialize it appropriately.
    output[(*index)++] = pow(offset + amplitude * sin(  tmp * i * dt + phase ), expon);
 else
  for (i=0; i<Ni; i++)              // I go through the entire data vector and I initialize it appropriately.
    output[(*index)++] = offset + amplitude * sin(  tmp * i * dt + phase );
